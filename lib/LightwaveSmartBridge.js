...
module.exports = class LightwaveSmartBridge extends EventEmitter
{
    ...
    async initialise()
    {
        try
        {
            ...
            Homey.app.updateLog( "Initialising Bridge" );
            ...
            if ( access_token != "" )
            {
                ...
                Homey.app.updateLog( "Bridge Initialised" );
    ...
    async initAccessToken()
    {
        if ( !Homey.ManagerSettings.get( 'bearerid' ) )
        {
            console.log( 'No bearer ID specified' );
        ...
        if ( !Homey.ManagerSettings.get( 'refreshtoken' ) )
        {
            console.log( 'No refresh token specified' );
    ...
    async getLocations()
    {
        ...
        {
            console.log( 'Failed to get Locations from Lightwave ' + err + ": " + result );
    ...
    async getDevices( locationID )
    {
        ...
        {
            console.log( 'Failed to get Devices from Lightwave' );
    ...
    async registerWEBHooks( featureID, feature, deviceID )
    {
        try
        {
            ...
            Homey.app.updateLog( "Registering LW Webhook", true );
            const bearerid = Homey.ManagerSettings.get( 'bearerid' );
            if ( bearerid == "" )
            {
                return "";
            ...
            const result = await this.sendMessage( 'post', '/v1/events', body, false );
            if ( result != -1 )
            {
                Homey.app.updateLog( "Registered LW Webhook", true );
                return result[ 'id' ];
            }
    ...
    async deleteLWWebhooks()
    {
        try
        {
            console.log( 'Deleting old LW WEBHooks', true );

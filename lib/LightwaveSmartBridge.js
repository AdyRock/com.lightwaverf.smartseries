...
module.exports = class LightwaveSmartBridge extends EventEmitter
{
    ...
    {
        var access_token = ManagerSettings.get('accesstoken');
        if (!access_token)
        {
            access_token = '';
        }
        ...
        if(access_token == '')
        {
		    throw new Error('Failed to connect to Lightwave to retrive a new access token');
        }
    ...
    async getNewTokens()
    {
		...
		const bearerid = ManagerSettings.get('bearerid');
		const refreshtoken = ManagerSettings.get('refreshtoken');
        ...
        try
        {
			...
			ManagerSettings.set('refreshtoken', connectResult['refresh_token']);
			ManagerSettings.set('accesstoken', access_token);
    ...
    async getLocations()
    {
        try
        {
            const result = await this.sendMessage('get', '/v1/structures', "");
            if (result != -1)
            {
                return result; 
            }
        ...
        {
            console.log('getLocations failed: ', result);
            throw new Error('Failed to get Locations from Lightwave');
        }
        return null;
	}
...
//            console.log('getDevices from location: ', locationID);
            ...
            const result = await this.sendMessage('get', '/v1/structure/' + locationID, "");
            if (result != -1)
            {
                return result; 
            }
        ...
        {
            console.log('getDevices failed: ', result);
    ...
    async getDevicesOfType(deviceType)
    {
        ...
        try
        {
            ...
            for (const location of lwstructures['structures'])
            {
                ...
                // loop through each device and check if it is a light
                for (const lwdevice of lwdevices['devices'])
                {
                    console.log('ForeEachDevice: ', lwdevice);
                    if (lwdevice['device'] == deviceType)
                    {
...
//            console.log('setFeature: ', featureId, " ", body);
            ...
            const result = await this.sendMessage('post', '/v1/feature/' + featureId, body);
            if (result != -1)
            {
                return result; 
            }
        ...
        {
            console.log('setFeature failed: ', result);
...
//            console.log('getFeature: ', featureId);
            ...
            const result = await this.sendMessage('get', '/v1/feature/' + featureId, "");
            if (result != -1)
            {
                return result['value']; 
            }
...
//                console.log('sendMessage Result: ', connectResult);
            ...
            {
                // Not an unortorized error so return now
                return -1;
            }
            ...
            )
    {
        ...
        {
            // Get the reply as text as it will posssibly be an error message
        ...
        if(!res.ok)
        {
            // The reult was bad so throw an error
//            console.log('status: ', res.status);
            const err = new Error((bodyText && bodyText.error && bodyText.error.message ) || (bodyText) || 'Unkown Lightwave Smart Error');
            err.code = res.status;
            throw err;
		}

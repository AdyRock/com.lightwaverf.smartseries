...
module.exports = class LightwaveSmartBridge extends SimpleClass
{
	...
	constructor(homey, webhookUrl)
	{
		...
		this.webhookRetrySpacingMs = 120;
		this.webhookRetryJitterMs = 180;
		this.featureValueCache = new Map();
		this.featureValueCacheTtlMs = 2 * 60 * 1000;
		this.startupFeatureCacheUntil = 0;
		this.startupFeatureCacheWindowMs = 60 * 1000;
		this.lastStartupCacheMissLog = 0;
		this.startupCacheMissLogPeriodMs = 10000;
		...
		this.missingFeatureCooldownUntilById = new Map();
		this.lastMissingFeatureLogById = new Map();
		this.missingFeatureCooldownMs = 10 * 60 * 1000;
		this.missingFeatureLogPeriodMs = 60 * 1000;
		this.lastFeatureReadUnavailableLog = 0;
		this.featureReadUnavailableLogPeriodMs = 10000;
		return this;
	}
	...
	_setFeatureCacheValue(featureId, value)
	{
		...
		this.featureValueCache.set(featureId,
			{
				value,
				timestamp: Date.now(),
			});
	...
	_getFeatureCacheValue(featureId)
	{
		if (typeof featureId !== 'string')
		{
			return null;
		...
		const cached = this.featureValueCache.get(featureId);
		if (!cached)
		{
			return null;
		}

		if ((Date.now() - cached.timestamp) > this.featureValueCacheTtlMs)
		{
			this.featureValueCache.delete(featureId);
			return null;
		}
	...
	_cacheBatchFeatureValues(valueList)
	{
		...
		const featureIds = Object.keys(valueList);
		for (const featureId of featureIds)
		{
			this._setFeatureCacheValue(featureId, valueList[featureId]);
		}
	}

	async _prefetchFeatureValuesForStartup()
	{
		this.startupFeatureCacheUntil = Date.now() + this.startupFeatureCacheWindowMs;

		const valueList = await this.getLWValues(false);
		if (!valueList || (typeof valueList !== 'object') || (Object.keys(valueList).length === 0))
		{
			this.homey.app.updateLog('Startup feature cache prefetch unavailable');
			return;
		}

		this._cacheBatchFeatureValues(valueList);
		this.homey.app.updateLog(`Startup feature cache primed (${Object.keys(valueList).length} features)`);
	}
	...
	_isMissingFeatureError()
	{
		if (!this.lastRequestError)
		{
			return false;
		}

		if (this.lastRequestError.statusCode !== 404)
		{
			return false;
		}

		const errorText = String(this.lastRequestError.message || '').toLowerCase();
		return errorText.indexOf('feature not found') >= 0;
	}

	_getFeatureIdFromFeaturePath(path)
	{
		...
		if (path.indexOf('/v1/feature/') !== 0)
		{
			return '';
		}

		return path.substring('/v1/feature/'.length);
	}

	_isFeaturePathMissingCooldown(featureId)
	{
		...
		const cooldownUntil = this.missingFeatureCooldownUntilById.get(featureId) || 0;
		return cooldownUntil > Date.now();
	}

	_logMissingFeatureCooldown(featureId)
	{
		if (typeof featureId !== 'string' || featureId === '')
		{
			return;
		...
		const now = Date.now();
		const lastLog = this.lastMissingFeatureLogById.get(featureId) || 0;
		if ((now - lastLog) < this.missingFeatureLogPeriodMs)
		{
			return;
		}

		const cooldownUntil = this.missingFeatureCooldownUntilById.get(featureId) || now;
		this.homey.app.updateLog(`Skipping stale feature endpoint /v1/feature/${featureId} until ${new Date(cooldownUntil).toISOString()}`);
		this.lastMissingFeatureLogById.set(featureId, now);
	}

	_markMissingFeaturePath(featureId)
	{
		if (typeof featureId !== 'string' || featureId === '')
		{
			return;
		...
		const cooldownUntil = Date.now() + this.missingFeatureCooldownMs;
		this.missingFeatureCooldownUntilById.set(featureId, cooldownUntil);
		this.homey.app.updateLog(`Detected stale feature endpoint /v1/feature/${featureId}. Suppressing retries until ${new Date(cooldownUntil).toISOString()}`);
		this.lastMissingFeatureLogById.set(featureId, Date.now());
	}

	_clearMissingFeaturePath(featureId)
	{
		if (typeof featureId !== 'string' || featureId === '')
		{
			return;
		...
		this.missingFeatureCooldownUntilById.delete(featureId);
		this.lastMissingFeatureLogById.delete(featureId);
	}

	_isWebhookAlreadyExistsError(path, errorText, statusCode)
	{
		...
		return (text.indexOf('notification already exists') >= 0)
			|| (text.indexOf('already exists') >= 0);
	}

	async _filterValidFeatureList(featureList)
	{
		const validFeatures = [];
		for (const feature of featureList)
		{
			...
			const value = await this.getFeatureValue(feature.featureId);
			if (value !== -1)
			{
	...
	{
		if ((typeof path === 'string')
			&& ((path.indexOf('/v1/feature/') === 0)
				|| (path.indexOf('/v1/features/read') === 0)))
		{
			return 'features';
		}
	...
	_isInServerCooldown(path = '')
	{
		...
		if (this.serverFailureCooldownScope === 'features')
		{
	...
	getServerCooldownRemainingMs(path = '')
	{
		...
		const remainingMs = this.serverFailureCooldownUntil - Date.now();
		if (remainingMs > 0)
		{
			return remainingMs;
		}
	...
	isBatchReadInCooldown()
	{
		return this._isInServerCooldown('/v1/features/read');
	}
	...
	_setServerFailureCooldown(reason, path = '')
	{
		...
		if ((now - this.lastServerFailureLog) > 10000)
		{
			...
			let scopeText = 'all endpoints';
			if (cooldownScope === 'events')
			{
				scopeText = 'events endpoint only';
			}
			else if (cooldownScope === 'features')
			{
				scopeText = 'feature endpoints only';
			}
	...
	async retryFailedWebhooks()
	{
		...
		for (const [deviceID, webhookInfo] of this.failedWebhookRegistrations)
		{
			if (!isFirstRetry)
			{
				const retryDelay = this.webhookRetrySpacingMs
					+ Math.floor(Math.random() * this.webhookRetryJitterMs);
				await this.asyncDelay(retryDelay);
			}
			isFirstRetry = false;
	...
	async initialise()
	{
		try
		{
			...
			if (accessToken !== '')
			{
				...
				try
				{
					await this._prefetchFeatureValuesForStartup();
				}
				catch (err)
				{
					this.homey.app.updateLog(`Startup feature cache prefetch failed: ${this._getErrorMessage(err)}`);
				}
	...
	async getDevicesOfType(deviceType)
	{
		...
		try
		{
			...
			{
				this.homey.app.updateLog(`Found Location: ${this.homey.app.varToString(location)}`);
	...
	async getFeatureValue(featureId, ValueList)
	{
		...
		if (ValueList)
		{
			if (Object.prototype.hasOwnProperty.call(ValueList, featureId))
			{
				const listValue = ValueList[featureId];
				this._setFeatureCacheValue(featureId, listValue);
				return listValue;
			}
			return -1;
		}

		if (Date.now() < this.startupFeatureCacheUntil)
		{
			const cachedStartupValue = this._getFeatureCacheValue(featureId);
			if (cachedStartupValue !== null)
			{
				return cachedStartupValue;
			}

			const now = Date.now();
			if ((now - this.lastStartupCacheMissLog) > this.startupCacheMissLogPeriodMs)
			{
				this.homey.app.updateLog(`getFeatureValue: startup cache miss for ${featureId}, skipping direct feature read`);
				this.lastStartupCacheMissLog = now;
			}
		...
		{
			const cachedCooldownValue = this._getFeatureCacheValue(featureId);
			if (cachedCooldownValue !== null)
			{
				return cachedCooldownValue;
			}
			return -1;
		}
		...
		if (result !== -1)
		{
			...
			{
				this._setFeatureCacheValue(featureId, result.value);
			}
	...
	async registerWEBHooks(featureID, feature, deviceID, isRetry = false)
	{
		...
		try
		{
			...
			if (result !== -1)
			{
				...
				{
					this.homey.app.updateLog(`Registered LW Webhook (${webhookNumber}): ${deviceID} (already existed)`);
				}
				else
				{
					this.homey.app.updateLog(`Registered LW Webhook (${webhookNumber}): ${deviceID}`);
				}
				return (result && result.id) ? result.id : deviceID;
			}
	...
	async getLWValues(allowMissingFeatureRepair = true)
	{
		if (this.isBatchReadInCooldown())
		{
			const now = Date.now();
			if ((now - this.lastFeatureReadUnavailableLog) > this.featureReadUnavailableLogPeriodMs)
			{
				const remainingMs = this.getServerCooldownRemainingMs('/v1/features/read');
				this.homey.app.updateLog(`getLWValues: batch read paused while API unavailable (${remainingMs} ms remaining)`);
				this.lastFeatureReadUnavailableLog = now;
			}
		...
		if (!this.featureList || this.featureList.length === 0)
		{
			...
			for (const location of lwstructures.structures)
			{
				...
				for (const lwdevice of lwdevices.devices)
				{
					...
					for (const lwfeatureset of lwdevice.featureSets)
					{
						// this.homey.app.updateLog(`ForeEachFeatureSet: ${this.homey.app.varToString(lwfeatureset)}`);
		...
		this.homey.app.updateLog(`LW Features count: ${this.featureList.length}`);

		// batch read the features
		const body = `{"features":${JSON.stringify(this.featureList, null, 2)}}`;
		let valueList = await this.sendMessage('post', '/v1/features/read', body, true);
		if ((valueList === -1) || !valueList)
		{
			...
			if (this._isMissingFeatureError())
			{
				if (!allowMissingFeatureRepair)
				{
					this.homey.app.updateLog('getLWValues: startup prefetch skipped stale-feature repair after Feature not found');
					return {};
				}

				const originalCount = this.featureList.length;
				const validFeatures = await this._filterValidFeatureList(this.featureList);
				if (validFeatures.length > 0 && (validFeatures.length < originalCount))
				{
					this.featureList = validFeatures;
					this.homey.app.updateLog(`getLWValues: removed ${originalCount - validFeatures.length} stale features from batch cache`);

					const retryBody = `{"features":${JSON.stringify(this.featureList, null, 2)}}`;
					valueList = await this.sendMessage('post', '/v1/features/read', retryBody, true);
					if ((valueList !== -1) && valueList)
					{
	...
	async sendMessage(method, path, body, ignoreReady)
	{
		let errMsg = '';
		this.lastRequestError = null;
		const featurePathId = this._getFeatureIdFromFeaturePath(path);
		if (this._isFeaturePathMissingCooldown(featurePathId))
		{
			this._logMissingFeatureCooldown(featurePathId);
			return -1;
		}
			...
			for (let i = 0; i < 3; i++)
			{
				try
				{
					...
					if (connectResult)
					{
						if (featurePathId)
						{
							this._clearMissingFeaturePath(featurePathId);
						}
				...
				catch (err)
				{
					...
					const statusCode = (typeof err.statusCode === 'number') ? err.statusCode : Number(err.statusCode);
					const status = Number.isFinite(statusCode) ? statusCode : 'n/a';

					if (this._isWebhookAlreadyExistsError(path, errText, statusCode))
					{
						...
						this.homey.app.updateLog(`Webhook already exists for ${path}. Treating as successful registration.`);
						return { existing: true, id: 'existing' };
					}

					this.homey.app.updateLog(`Connection Failed (${method.toUpperCase()} ${path}): ${err}`);
					const requestId = this._getResponseRequestId(err) || 'n/a';
					const headers = this._getHeaderSummary(err);
					const bodyPreview = this._getResponseBodyPreview(err) || 'n/a';
					this.homey.app.updateLog(`Response details (${method.toUpperCase()} ${path}): error=${errText}, status=${status}, requestId=${requestId}, headers=${headers}, body=${bodyPreview}`);
					this.lastRequestError = {
						message: errText,
						statusCode: status,
						requestId,
					...
					if (featurePathId && statusCode === 404 && errMsg.indexOf('feature not found') >= 0)
					{
						this._markMissingFeaturePath(featurePathId);
						return -1;
					}
	...
	async fetch(url, opts, body)
	{
		return new Promise((resolve, reject) =>
		{
			try
			{
				...
				req.on('error', (err) =>
				{
					const netErr = new Error(`HTTPS Catch: ${err}`);
					netErr.code = 'network_error';
					netErr.statusCode = 'network_error';
					netErr.responseBody = '';
					netErr.responseHeaders = {};
					reject(netErr, 0);
				});
				...
				req.setTimeout(15000, () =>
				{
					...
					timeoutErr.statusCode = 'timeout';
					timeoutErr.responseBody = '';
					timeoutErr.responseHeaders = {};
					reject(timeoutErr);
				});

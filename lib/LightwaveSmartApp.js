...
class LightwaveSmartApp extends Homey.App
{
    ...
    async registerHomeyWebhook()
    {
        try
        {
            if ( await this.lwBridge.waitForBridgeReady() )
            {
                ...
                this.updateLog( "Homey WebHook data: " + JSON.stringify( data, null, 2 ) );
                ...
                return this._webhook.register()
                    ...
                    .catch( ( err ) =>
                    {
                        this.updateLog( "Register Homey WEBHook Error: " + err );
                    } )
            }
        }
        catch ( err )
    ...
    async onPoll()
    {
        ...
        try
        {
            if ( Homey.ManagerSettings.get( 'usePolling' ) )
            {
                ...
                for ( const driver in drivers )
                {
                    let devices = Homey.ManagerDrivers.getDriver( driver ).getDevices(); //.forEach( device =>
                    for ( var i = 0; i < devices.length; i++ )
                    {
        ...
        if ( Homey.ManagerSettings.get( 'usePolling' ) )
        {
            ...
            nextInterval += ( promises.length * 200 );
    ...
    async getSomething( args )
    {
        ...
        if (args.query && args.query.cmd && args.query.cmd == "listEvents" )
        {
    ...
    addSomething( args )
    {
        this.updateLog( "POST: " + JSON.stringify( args, null, 2 ) );
        return "OK";
    }

    updateSomething( args )
    {
        this.updateLog( "PUT: " + JSON.stringify( args, null, 2 ) );
        return "OK";
    }

    deleteSomething( args )
    {

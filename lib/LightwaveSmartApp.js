...
class LightwaveSmartApp extends Homey.App
{
    ...
    parseSupportMessage(message)
    {
        ...
        const apiUnavailableMatch = message.match(/^Lightwave API unavailable\. Pausing requests until ([^.]+)\. Reason: (.*)$/);
        if (apiUnavailableMatch)
        {
            details.type = 'apiUnavailable';
            details.pausedUntil = apiUnavailableMatch[1].trim();
            details.reason = apiUnavailableMatch[2].trim();
            return details;
        }
    ...
    updateLog(newMessage, webHookMessage = false)
    {
        ...
        const alwaysLog = /error|failed|failure|timeout|unavailable/i.test(messageText);
